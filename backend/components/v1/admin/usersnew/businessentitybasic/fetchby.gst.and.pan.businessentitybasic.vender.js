...
const Gst = require('../../../../../models/be_gst_details');
const GstPerson = require('../../../../../models/be_gst_person_assigned');
const License = require('../../../../../models/license_details');
  ...
  try {
    ...
    if (be_information) {
      ...
      const [
        ...
        (async () => {
          try {
            return await Bankdetails.where({ be_information_id }).fetchAll({ require: false });
          } catch (err) {
            return [];
          }
        })(),
        (async () => {
          try {
            return await Assigned.where({ be_information_id }).fetchAll({ require: false });
          } catch (err) {
            return [];
          }
        })(),
        (async () => {
          try {
            return await License.where({ be_information_id }).fetchAll({
              require: false,
              withRelated: [
                'signatureandseal',
                'license',
        ...
        (async () => {
          try {
            return await Warehouse.where({ be_information_id }).fetchAll({ require: false });
          } catch (err) {
            return [];
          }
        })()
      ...
      let be_license_details = [];

      if (license && license.length > 0) {
        be_license_details = license.toJSON().map((item, index) => {
          return {
            ...item,
            signatureandseal: item.signatureandseal?.photo_path || null,
            license: item.license?.photo_path || null
          };
        });
      }
      ...
      return res.success({
        ...
        // be_owner_details: ownerDetails,
        be_document: uploadedFiles,
        be_license_details: be_license_details,
        be_warehouse_information: warehouse,
      });
    } else {
      return res.success({ message: "Business Entity Added Successfully." });
    }
