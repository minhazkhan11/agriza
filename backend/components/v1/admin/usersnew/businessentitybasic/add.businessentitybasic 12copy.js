...
//             savedPerson = existingUsers.find(user => user.get("phone") === person.phone);
...
//             };
//             if (person.password) {
//               const hashedPassword = await bcrypt.hash(person.password, 10);  // Hash with saltRounds = 10
...
//           await Beassigned.where({ be_information_id: savedRecord.id, user_id: savedPerson.id }).fetch({ require: false })
