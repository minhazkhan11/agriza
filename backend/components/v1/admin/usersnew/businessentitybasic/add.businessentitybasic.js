        ...
        for (const person of personsData) {
          ...
          if (person.id) {
            ...
            if (savedPerson) {
              ...
              const currentRole = savedPerson.get('role');
              const currentStatus = savedPerson.get('active_status');
              const currentplanid = savedPerson.get('menu_plan_id');
              ...
              if (person.admin === "yes") {
                updateFields.active_status = "active";
                updateFields.role = "admin";
              }

              else if (person.admin === "no") {

                if (currentRole === 'user' && currentStatus === 'active') {
                  updateFields.active_status = "active";
                  updateFields.role = currentRole; // keep role
                  updateFields.menu_plan_id = currentplanid; // keep role
                } else {

                  updateFields.active_status = "inactive";
                  updateFields.role = "admin";
                }
              }


              if (currentRole === 'user' && currentStatus === 'active' && person.admin === 'yes') {
                updateFields.role = "admin";
              }
          ...
          const existingAssignment = await Beassigned.where({
