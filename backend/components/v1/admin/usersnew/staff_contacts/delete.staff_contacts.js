...
module.exports = async (req, res, next) => {
  try {
    ...
    const check = await User.query((qb) => {
      qb.where({ id: req.params.id }).whereIn('role', ['procurement', 'salesman']);
    }).fetch({ require: false })
    if (!check)
      return res.serverError(400, ErrorHandler(new Error('staff_contact details not found')));
    await new User().where({ id: req.params.id }).save({ active_status: constants.activeStatus.deleted }, { method: 'update' })
      .then(() => {
        return res.success({ 'message': 'staff_contact details deleted successfully' });
      })
      .catch(err => {
