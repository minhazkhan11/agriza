...
module.exports = async (req, res) => {
  try {
    ...
    const assignedData = {};
    if (userBody.business_area_zone !== undefined)
      assignedData.business_area_zone = String(userBody.business_area_zone);
    if (userBody.business_area_id !== undefined) assignedData.business_area_id = JSON.stringify(userBody.business_area_id);
    if (userBody.warehouse_id !== undefined) assignedData.warehouse_id = JSON.stringify(userBody.warehouse_id);
    if (userBody.gst_id !== undefined) assignedData.gst_id = JSON.stringify(userBody.gst_id);
    if (userBody.vendor_id !== undefined) assignedData.vendor_id = JSON.stringify(userBody.vendor_id);
    if (userBody.customer_id !== undefined) assignedData.customer_id = JSON.stringify(userBody.customer_id);
    if (userBody.be_information_id !== undefined)
      assignedData.be_information_id = (userBody.be_information_id);
    // if (userBody.Effective_date_change !== undefined) assignedData.Effective_date_change = userBody.Effective_date_change;
    if (Object.keys(assignedData).length > 0) {
      assignedData.updated_at = new Date();
      let assignedRecord = await Assigned.where({ user_id: user.id }).fetch({ require: false });
      if (assignedRecord) {
        await assignedRecord.save(assignedData, { patch: true });
      } else {
