...
module.exports = async (req, res, next) => {
    try {
        // **Fetch all product_area records**
        const product_areas = await ProArea.query((qb) => {
            ...
            , columns: ['id', 'demographic_includes_id', 'marketer_id', 'demographic_include', 'active_status']
        });
        ...
        for (const product_area of product_areas.toJSON()) {
            ...
            const includeId = product_area.demographic_includes_id;
            const excludeType = product_area.demographic_exclude;
            const excludeId = product_area.demographic_excludes_id;
            const excludeType1 = product_area.demographic_exclude_2;
            const excludeId1 = product_area.demographic_exclude_2_id;
            ...
            if (excludeType1 === 'State') {
                productData.exclude_State = await fetchDemographics(State, excludeId1);
            } else if (excludeType1 === 'District') {
                productData.exclude_District = await fetchDemographics(District, excludeId1);
            } else if (excludeType1 === 'Pincode') {
                productData.exclude_Pincode = await fetchDemographics(Pincode, excludeId1);
            } else if (excludeType1 === 'Tehsil') {
                productData.exclude_Tehsil = await fetchDemographics(Tehsil, excludeId1);
            } else if (excludeType1 === 'Place') {
                productData.exclude_Place = await fetchDemographics(Place, excludeId1);
            }
            else if (excludeType1 === 'Country') {
                productData.exclude_Country = await fetchDemographics(Country, excludeId1);
            }
