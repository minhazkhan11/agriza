...
module.exports = async (req, res, next) => {
    try {
        const product_area = await ProArea.query((qb) => {
        ...
        if (!product_area) {
            return res.serverError(400, 'Invalid product_area');
        }

        let responseData = {
            success: true,
            product_area: product_area.toJSON()
        };
        ...
        const demographicType = product_area.get('demographic_include');
        const demographicIds = parseIds(product_area.get('demographic_includes_id'));

        const demographicType1 = product_area.get('demographic_exclude');
        const demographicIds1 = parseIds(product_area.get('demographic_excludes_id'));

        const demographicType2 = product_area.get('demographic_exclude_2');
        const demographicIds2 = parseIds(product_area.get('demographic_exclude_2_id'));
