    ...
    try {
        const orders = await Order.query(qb => {
            qb.where('order_type', 'so')
                .andWhere(function () {
                    this
                        // include any SO that's NOT dropshipping
                        .whereNot('ship_type', 'dropshipping')
                        // OR include dropshipping SO whose status IS in the allowed list
                        .orWhere(function () {
                            this.where('ship_type', 'dropshipping')
                                .whereIn('order_status', ['dispatch', 'delivered', 'rejected']);
                        });
                });
        }).fetchAll({
            require: false,
            withRelated: [
                ...
                {
                    customer_ship_to_party: qb => qb.select(
                        'id', 'warehouse_name', 'warehouse_address', 'business_name', 'gst_no'
                    )
                }
            ]
        });
        ...
        return res.success({
            success: true,
            message: 'Filtered SO orders fetched successfully',
            orders: result
        });
