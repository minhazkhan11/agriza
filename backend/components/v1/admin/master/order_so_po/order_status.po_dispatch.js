...
module.exports = async (req, res) => {
    try {
        // Fetch only orders where order_type is 'po' and order_status is 'dispatch'
        const orders = await Order.query(qb => {
            qb.where('order_type', 'po')
                .andWhere('order_status', 'dispatch');
        }).fetchAll({
            require: false,
            withRelated: [
                {
        ...
        return res.success({
            success: true,
            message: 'All PO orders with dispatch status fetched successfully',
            orders: result
        });
