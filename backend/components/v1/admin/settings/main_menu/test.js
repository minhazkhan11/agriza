...
module.exports = async (req, res) => {
  try {
    let { menu_plan_id, menu_plan_id_update, menu } = req.body;
    ...
    for (let menuItem of menu) {
      ...
      if (menuItem.menu_type === "main") {
        ...
        const updateexistingMenu = existingMenu
          ? await MainMenu.where(qb => {
            qb.where({ name: existingMenu.get('name'), menu_plan_id: menu_plan_id })
              .orWhere({ name: existingMenu.get('name'), menu_plan_id: menu_plan_id_update });
          }).fetch({ require: false })
          : null;
      ...
      if (menuItem.menu_type === "sub") {
        ...
        let existingSubMenuPlan = await Submenu.where(qb => {
          qb.where({ name: existingSubMenu.get('name'), main_menu_id: newMainMenuId })
            .orWhere({ name: existingSubMenu.get('name'), menu_plan_id: menu_plan_id_update });
        }).fetch({ require: false });
        ...
        let existingChildMenuPlan = await Childmenu.where(qb => {
          qb.where({ sub_menu_id: newSubMenuId, name: existingChildMenu.get('name') })
            .orWhere({ name: existingChildMenu.get('name'), menu_plan_id: menu_plan_id_update });
        }).fetch({ require: false });
