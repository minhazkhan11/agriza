...
module.exports = async (req, res) => {
  try {
    ...
    const filePath = path.join(exportPath, 'vender_export.csv');
    ...
    for (const record of records) {
      try {
        ...
        const savedCustomer = await new VenderLead(vanderData).save();
        processedRecords.push(savedCustomer.toJSON());
      } catch (error) {
        console.error('Error processing vender:', error.message);
    ...
    return res.json({
      message: 'Vender imported and exported successfully',
      count: processedRecords.length,
      relativeExportPath: `/uploads/exports/vender_export.csv`, // Web-accessible path
    ...
    console.error('Error in vender import/export:', error);
