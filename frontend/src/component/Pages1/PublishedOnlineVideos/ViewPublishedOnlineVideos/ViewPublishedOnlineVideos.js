...
import EditPublishedVideo from "../EditPublishedVideo/EditPublishedVideo";

function ViewPublishedOnlineVideos() {
...
const [onlinevideo,setOnlinevideo]=useState([]);
  ...
  const fetchTestSeriesData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/onlinevideo/publish/onlinevideo`,
        {
          headers: {
      ...
      if (response.data.success === true) {
        ...
        setOnlinevideo(response.data.onlinvideo);
      }
    } catch (error) {
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/onlinevideo/publish/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
  ...
  const rows = onlinevideo.map((d) => ({
    id: d.id ? d.id : "N/A",
    name: d.video_name ? d.video_name : "N/A",
    exam: d.exam.exam_name ? d.exam.exam_name : "N/A",
  ...
  const columns = [
    ...
    },
    {
      ...
      headerAlign: "center",
      autoPageSize: false,
      customBodyRenderLite: (cellValues, rowIndex) => {
        console.log("Status:", cellValues.row.status); // Check the value of status
        return (
          <>
            {cellValues.row.status == "inactive"
              ...
              ? "Published request"
              : cellValues.row.status == "refused"
              ? "Refused"
              : cellValues.row.status}
            {cellValues.row.status && <div>({cellValues.row.status})</div>}
          </>
        );
      },
        ...
        return (
          <div className={`${classes.publishactionpaper} ${classes.dflex} ${classes.alignitemscenter} ${classes.justifycenter} ${classes.mr1}`}>
            { ! isActive ? (
                      <Paper>
                      <IconButton
                      onClick={()=>handleStatus(cellValues.row.id)}>
                      ...
                      <IconButton
                        onClick={() => {
                          handleButtonClick("editpublishedvideolecture", cellValues.row.id);
                        }}
                      >
...
export default ViewPublishedOnlineVideos;
