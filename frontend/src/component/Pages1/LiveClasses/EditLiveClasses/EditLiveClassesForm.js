...
const EditLiveClassesForm = () => {
  ...
  const handleTypeChange = (event) => {
    ...
    console.log(event.target.value, "typetype");
  };
  ...
  const fetchData = async () => {
    try {
      ...
      const data = response.data.liveclassData;

      setType(data?.type);
      setName(
        data?.live_class_name ? data?.live_class_name : data?.webinar_name
      );
      setCode(data?.live_class_code);
      setSelectedCourse(data?.course_id);
      setSelectedBatch(data?.batch_id);
      setSelectedSubject(data?.subject_id);
      setLiveDate(data?.date ? data?.date.split("T")[0] : "");
      ...
      setZooMetId(data?.zoom_meeting_id);
      setZooPass(data?.zoom_password);
      setDescription(data?.description);
      setCheck(data?.notify_user);
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      if (response.data.success === true) {
        ...
        setTimeout(() => {
          if (type === "webinar") {
            ...
            navigate(
              `/admin/liveclassesschedule/${response.data?.live_class?.id}`
            );
          }
        }, 2000);
      }
    } catch (error) {
      let errorMessage = `Failed to update ${typeName}.`;
      ...
      console.error("Error in updating live class:", error);
...
export default EditLiveClassesForm;
