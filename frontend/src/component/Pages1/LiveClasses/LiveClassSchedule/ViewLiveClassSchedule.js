...
function ViewLiveClassSchedule({ liveClassesDetails }) {
  ...
  const fetchScheduleByLiveClassId = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/liveclasslectures/by_live_class_id/${rowId}`,
        {
          method: "GET",
          headers: {
  ...
  const deleteContent = async (scheduleID) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/liveclasslectures/${scheduleID}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
...
export default ViewLiveClassSchedule;
