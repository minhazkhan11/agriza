...
function EditNotesForm() {
  ...
  const handleFormSubmit = async () => {
    // Ensure clientName and formDetails.code are defined and not just whitespace
  ...
  const fetchDataFromAPI = async (rowId) => {
    try {
      ...
      if (response.status === 200 && response.data.success) {
        ...
        // Updating the clientName to the name of the note
        setClientName(noteData.name);

        // Updating formDetails with the note's details
        ...
        // Updating the selected course, batch, and subject with their respective IDs
        setSelectedCourse(noteData.course_id);
        setSelectedBatch(noteData.batch_id);
        setSelectedSubject(noteData.subject_id);

        // Setting the thumbnail image and its preview
        setThumbnailImage(noteData.image_url);
        setThumbnailImagePreview(noteData.image_url);
      } else {
