...
function ViewNotesName({ notename }) {
  ...
  const fetchContentByNoteId = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/note_contents/by_note_id/${rowId}`,
        {
          method: "GET",
          headers: {
  ...
  const deleteContent = async (contentID) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/note_contents/${contentID}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
  ...
  const handleStatusForContent = (contentId, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/note_contents/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
  ...
  const renderMediaContent = (fileFormat, contentUrl) => {
    ...
    switch (fileFormat) {
      ...
      case "pdf":
        return (
          <iframe src={contentUrl} title="PDF" width="100%" height="500px" />
        );
      case "image":
        return (
          <img
  ...
  return (
    <>
      ...
      <div className={` ${classes.mt1} ${classes.p1} ${classes.h60vh}`}>
        <div
