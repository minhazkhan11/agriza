...
function EditSingleTest({
  ...
  // const [names, setNames] = useState(["English"]);
  // const [personName, setPersonName] = React.useState(["English","Hindi"]);
  ...
  const fetchLanguages = async (rowId) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/testseries/languages/${rowId}`,
        {
          headers: {
  ...
  const handleSubmit = () => {
    ...
    fetch(`${process.env.REACT_APP_API_BASE_URL}/v1/b2b/testseries_questions`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${decryptedToken}`,
  ...
  const fetchDataFromAPI = async (questionId) => {
    try {
      const apiUrl = `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/testseries_questions/${questionId}`;
      ...
      if (response.status === 200 && response.data.success) {
        ...
        const updatedOptionsEnglish = questionData.options.map(
          (option, index) => ({
            id: option.id,
            index,
        ...
        const updatedOptionsHindi = questionData.options.map(
          (option, index) => ({
            id: option.id,
            index,
        ...
        // Set the selected correct option if any
        const correctOptionIndex = questionData.options.findIndex(
          (opt) => opt.correct_option
        );
        if (correctOptionIndex !== -1) {
          setSelectedCorrectOption(alphabetArray[correctOptionIndex]);
        }
        ...
        if (correctOption) {
          setQuillContentEnglishSolution(correctOption.solution_en || "");
          setQuillContentHindiSolution(correctOption.solution_hi || "");
        }
      } else {
