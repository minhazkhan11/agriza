...
function EditTestSeriesForm() {
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      if (response.data.success === true) {
        ...
        navigate(`/admin/addquestiontestseries/${rowId}`);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (formDetails.no_of_question && formDetails.total_score) {
      const calculatedMarks =
        formDetails.total_score / formDetails.no_of_question;
      setPerQuestionMarks(calculatedMarks.toFixed(2));
    }
  }, [formDetails.no_of_question, formDetails.total_score]);
  ...
  const fetchDataFromAPI = async (rowId) => {
    try {
      const apiUrl = `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/testseries/${rowId}`;
      ...
      if (response.status === 200 && response.data.success) {
        const ts = response.data.test_series;
  
        // Setting the form details
        setFormDetails({
          test_series_name: ts.name,
          code: ts.code,
          description: ts.description,
          start_date: ts.start_date.split("T")[0], // Assuming start_date is in ISO format and you need only the date part
          start_time: ts.start_time,
          language: ts.language.map((lang) => lang.code),
          duration: ts.duration.toString(), // Ensure it's a string if your TextField expects a string
          total_score: ts.total_score.toString(), // Convert to string if necessary
          no_of_question: ts.no_of_question.toString(), // Convert to string if necessary
          per_question_marks: (ts.total_score / ts.no_of_question).toFixed(2), // Calculate and convert to string
        });
  
        // Setting the selected course, batch, and subjects
        setSelectedCourse(ts.course_id);
        setSelectedBatch(ts.batch_id);
        setSelectedSubject(ts.subjects.map((subject) => subject.id)); // Assuming you need the IDs in an array
  
        // Setting the thumbnail image
        setThumbnailImage(ts.image_url);
        setThumbnailImagePreview(ts.image_url);
      } else {
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        Test Series Name <span className={classes.textcolorred}>*</span>
                      </FormLabel>
                      <TextField
                        type="text"
                        variant="outlined"
                  ...
                  >
                    ...
                    >
                      <FormLabel
                        className={`${classes.textcolorformlabel} ${classes.fontfamilyoutfit}  ${classes.fontsize1} ${classes.fontstylenormal} ${classes.fw400} ${classes.lineheight}`}
                      >
                        Questions Count <span className={classes.textcolorred}>*</span>
                      </FormLabel>
                      <TextField
                        type="number"
                        variant="outlined"
                    ...
                    >
                      <FormLabel
                        className={`${classes.textcolorformlabel} ${classes.fontfamilyoutfit}  ${classes.fontsize1} ${classes.fontstylenormal} ${classes.fw400} ${classes.lineheight}`}
                      >
                        Per Questions Marks <span className={classes.textcolorred}>*</span>
                      </FormLabel>
                      <TextField
                        type="number"
                        variant="outlined"
