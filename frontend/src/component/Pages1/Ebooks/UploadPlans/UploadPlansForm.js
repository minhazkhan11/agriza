...
function UploadPlansForm({ ebookanme }) {
  ...
  const fetchUploadPlans = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/ebook_upload_plans/by_ebookid/${rowId}`,
        {
          method: "GET",
          headers: {
  ...
  const deleteContent = async (contentID) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/ebook_upload_plans/${contentID}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
  ...
  const handleStatusForContent = (contentId, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/ebook_upload_plans/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
