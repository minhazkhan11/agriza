...
function AddSingle({ changeTab }) {
  ...
  const handleSubmit = () => {
    ...
    let updatedOptionsArray = optionsArrayEnglish.map((optionEng, index) => {
      ...
      // Default correct option logic for "Fill In The Blanks"
      let isCorrect =
        formDetails.questiontype === "Fill In The Blanks" && index === 0
          ? true // Set first option as correct
          : optionEng.correct_option || hindiOption.correct_option; // Keep existing logic for other question types
            ...
            <div className={`${classes.dflex} ${classes.justifyspacebetween} `}>
              ...
              >
                <FormLabel
                  className={`${classes.textcolorformlabel} ${classes.fontfamilyoutfit}  ${classes.fontsize1} ${classes.fontstylenormal} ${classes.fw400} ${classes.lineheight}`}
                >
                  Marks <span className={classes.textcolorred}>*</span>
                </FormLabel>
                <TextField
                  type="number"
                  variant="outlined"
