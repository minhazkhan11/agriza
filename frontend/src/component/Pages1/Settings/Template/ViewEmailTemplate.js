...
function ViewEmailTemplate({ handleChange }) {
  const style = [
    {
      style: "viewtable",
      height: "h54vh",
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/templateemail`,
        {
          headers: {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/templateemail/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      toast.error("Email template is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/templateemail/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Email template status changed successfully", response);
        fetchData();
        toast.success("Email template status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Email template status:", error);
        toast.error("Email template status is not changed");
      });
  };
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={(e) => handleChange(e, 7, cellValues.row.id)}
                >
