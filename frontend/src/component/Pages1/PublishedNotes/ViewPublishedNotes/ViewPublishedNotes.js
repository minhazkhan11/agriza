...
import EditPublishedNotes from "../EditPublishedNotes/EditPublishedNotes";

function ViewPublishedNotes() {
  ...
  const fetchTestSeriesData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/notes/publish/note`,
        {
          headers: {
      ...
      if (response.data.success === true) {
        ...
        setTestSeriesData(response.data?.notes);
      }
    } catch (error) {
...
useEffect(()=>{
  ...
  const handleStatus = (iddd) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/notes/publish/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Notes status changed successfully", response);
        fetchTestSeriesData();
        toast.success("Notes Publish successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Test Series status:", error);
        toast.error("Notes status is not changed");
      });
  };

  const rows = testSeriesData.map((d) => ({
    id: d.id ? d.id : "N/A",
    name: d.note_name ? d.note_name : "N/A",
    section: d.section.section_name ? d.section.section_name : "N/A",
  ...
  const filteredRows = rows
    .filter((d) => {
      ...
      const isCodeMatch = d.section.toLowerCase().includes(query);
  ...
  const columns = [
    ...
    {
      field: "status", // Adjust field to ensure uniqueness
      headerName: "Status",
      headerClassName: "super-app-theme--header",
      width: 150,
      flex: 1,
    ...
    },
    {
      ...
      renderCell: (cellValues) => {
        ...
        return (
          <div className={`${classes.publishactionpaper} ${classes.dflex} ${classes.alignitemscenter} ${classes.justifycenter} ${classes.mr1}`}>
            {! isActive ? (
              <Paper>
                <IconButton>
                  <LightTooltip title="Publish"onClick={()=>handleStatus(cellValues.row.id)}>
                    <TurnedInNotIcon />
                  </LightTooltip>
                </IconButton>
                <IconButton onClick={() => handleButtonClick("editnotescontent", cellValues.row.id)}>
                  <LightTooltip title="Edit Content">
                    <AddCircleOutlineIcon />
                  </LightTooltip>
                </IconButton>
                <IconButton onClick={() => handleOpenClose(cellValues.row.id)}>
  ...
  const Heading = [
    {
      ...
      inputplaceholder: "Search Published Notes",
      exportimport: "yes",
    },
  ];
...
export default ViewPublishedNotes;
