...
function EditBookForm() {
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/books`,
        formData,
        {
          headers: {
      ...
      toast.error(error.response.data.message || "Book is not updated !");
    }
  };
  ...
  const fetchDataFromAPI = async (rowId) => {
    try {
      const apiUrl = `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/books/${rowId}`;
      ...
      if (response.status === 200) {
        ...
        const country_of_origin = {
          name: data.country_of_origin,
        };

        setCountryOfOrigin(country_of_origin);

        // Set the initial state for images and imagePreviews
        const initialImages = data.images.map((imageUrl) =>
          fetch(imageUrl).then((response) => response.blob())
        );

        const initialImagePreviews = await Promise.all(initialImages);

        setImages(initialImagePreviews);
  ...
  return (
    <>
      <ToastContainer />
      <div
        className={`${classes.mt1} ${classes.mt1} ${classes.inputpadding} ${classes.inputborder} ${classes.pagescroll} ${classes.h75vh}`}
      >
