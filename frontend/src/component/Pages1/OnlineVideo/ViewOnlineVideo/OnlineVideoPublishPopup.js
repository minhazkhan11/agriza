...
const OnlineVideoPublishPopup = (props) => {
...
const [yourCost,setYourCost]=useState("");
  ...
  const fetchContentByNoteId = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/onlinevideolectures/by_onlinevideo_id/${info}`,
        {
          method: "GET",
          headers: {
  ...
  const handleSubmit = () => {
    ...
    axios({
      method: "post",
      url: `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/content/onlinevideo/onlinevideos/add/${info}`,
      data,
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
export default OnlineVideoPublishPopup;
