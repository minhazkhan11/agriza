...
function ComposeMailLearner() {
  ...
  const fetchLearners = async (selectedCourse, selectedBatch) => {
    const requestData = {
      user: {
  ...
  const handleFormSubmit = async () => {
    setLoading(true); 
    const emailArray = selectedLearner.length === 0
              ...
              >
                <FormLabel
                  className={`${classes.textcolorformlabel} ${
                    classes.fontfamilyoutfit
                  } ${classes.fontsize1} ${classes.fontstylenormal} ${
                    classes.fw400
                  } ${classes.lineheight} ${
                    !sendLater && classes.disabledLabel
                  }`}
                >
                  Schedule Date
                </FormLabel>
...
export default ComposeMailLearner;
