...
import EditPublishedLiveclasses from "../EditPublishedLiveClasses/EditPublishedLiveClasses";

function ViewPublishedLiveClasses() {
  ...
  const fetchLiveClassData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/liveclasses/publish/liveclass`,
        {
          headers: {
      ...
      if (response.data.success === true) {
        setLiveClassData(response?.data?.liveclass);
      }
    } catch (error) {
  ...
  const handleStatus = (iddd) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/b2b/liveclasses/publish/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        fetchLiveClassData();
        toast.success("Live Classes Publish status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Live Classes status:", error);
        toast.error("Live Classes Publish status is not changed");
      });
  };

  const rows = liveClassData.map((d) => ({
    id: d.id ? d.id : "N/A",
    name: d.live_class_name ? d.live_class_name : d.webinar_name,
    exam: d.exam && d.exam.exam_name ? d.exam.exam_name : "N/A",
    type: d.type === "live_class" ? "live class" : "webinar",
  ...
  const filteredRows = rows
    .filter((d) => {
      ...
      return isNameMatch || isCodeMatch || isDurationMatch || isStartDateMatch;
    })
    .map((row, index) => ({
      ...row,
      srno: index + 1,
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (cellValues) => {
        const isActive =
          cellValues.row.status === "Published request" ||
          cellValues.row.status === "Published";
            ...
            {!isActive ? (
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick(
                      "editpublishedschedules",
                      cellValues.row.id
                    );
                  }}
                >
                  <LightTooltip title="Edit Schedules">
  ...
  return (
    <>
      ...
      >
        <EditPublishedLiveclasses
          handleOpenClose={handleOpenClose}
          open={open}
          rows={rows}
          info={info}
          handleClose={handleClose}
          fetchLiveClassData={fetchLiveClassData}
        />
      </Modal>
    </>
  );
}
export default ViewPublishedLiveClasses;
