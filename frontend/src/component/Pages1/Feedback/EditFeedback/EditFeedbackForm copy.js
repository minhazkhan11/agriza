...
function EditFeedbackForm() {
  ...
  console.log(quillInputs,"data")
  useEffect(() => {
    fetchData(rowId);
  }, [rowId,feedBack]);
  const handleAddInput = () => {
    const newId = quillInputs.length + 1;
    setQuillInputs([...quillInputs, { question: "" }]);
};
  ...
  const handleRemoveInput = async (idToRemove) => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/v1/b2b/feedback/question/${idToRemove}`,
      {
        headers: {
          Authorization: `Bearer ${decryptedToken}`,
        },
    ...
    try {
      const data = {
        feedback: {
          ...
          schedule_date: date,
          questions: quillInputs.map((pair) => ({
            id: pair.id,
