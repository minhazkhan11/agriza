...
function TeacherTableViewSearch(props) {
  const classes = useStyles();
  const { fetchData,teacher ,decryptedToken,  Heading, onSearch } = props;
  ...
  const exportApi = async (decryptedToken) => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/v1/b2b/teacher/export/all`, {
        headers: {
  ...
  useEffect(() => {
    if (teacher && decryptedToken) {
      exportApi(decryptedToken);
    }
  }, [teacher, decryptedToken]);
  ...
  const handleFileInputChange = async (e) => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/v1/b2b/teacher/import/csv`, formData, {
      ...
      if (error.response && Array.isArray(error.response.data)) {
        error.response.data.forEach((err) => {
          const errorMessage = Object.entries(err).map(([key, value]) => `${key}: ${value}`).join(', ');
          toast.error(errorMessage);
        });
      } else {
        fetchData();
        ...
        const errorMessage = error.response && error.response.data.message ? error.response.data.message : 'An error occurred while uploading the file';
        toast.error(errorMessage);
      }
    }
};
      ...
      {Heading?.map((data) => (
        ...
        >
          ...
          >
            ...
            <div className={`${classes.dflex}`}>
              <TextField
                className={`${classes.textcolorformlabel} ${classes.searchinputfontsize}`}
                type="text"
                variant="outlined"
                required
...
export default TeacherTableViewSearch;
