...
const filterMenu = (menu, searchTerm, setDropdownsToOpen) => {
  let dropdownsToOpen = new Set();
  ...
  const filteredMenu = menu
    .map((item, index) => {
      if (item.name.toLowerCase().includes(searchTerm.toLowerCase())) {
        ...
        const filteredSubMenu = filterMenu(item.menu, searchTerm, setDropdownsToOpen);
        if (filteredSubMenu.length > 0) {
          dropdownsToOpen.add(index); // Store index of matching parent dropdown
          return { ...item, menu: filteredSubMenu };
        }
      } else if (item.child_menu) {
        const filteredChildMenu = filterMenu(item.child_menu, searchTerm, setDropdownsToOpen);
        if (filteredChildMenu.length > 0) {
          dropdownsToOpen.add(index);
          return { ...item, child_menu: filteredChildMenu };
        }
      }
      return null;
    })
  ...
  setDropdownsToOpen(dropdownsToOpen);
  return filteredMenu;
};
...
export default function NestedList({ MainHeading, menuCollapse }) {
  ...
  const [dropdownsToOpen, setDropdownsToOpen] = useState(new Set());
  ...
  const filteredMenu = filterMenu(MainHeading, searchTerm, setDropdownsToOpen);

  useEffect(() => {
    if (searchTerm.length > 0) {
      setOpenDropdownIndex([...dropdownsToOpen][0] ?? null);
    } else {
      setOpenDropdownIndex(null);
      setOpenSubDropdownIndex(null);
    }
  }, [searchTerm, dropdownsToOpen]);
