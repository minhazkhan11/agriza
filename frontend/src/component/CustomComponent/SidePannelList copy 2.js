...
export default function NestedList({ MainHeading, menuCollapse }) {
  ...
  const filterMenu = (menu, parentMatch = false) => {
    return menu
      .map((item, index) => {
        let isMatch = item.name.toLowerCase().includes(searchTerm.toLowerCase());
        let filteredSubMenu = [];
        let filteredChildMenu = [];

        if (item.menu) {
          filteredSubMenu = filterMenu(item.menu, isMatch);
          if (filteredSubMenu.length > 0) isMatch = true; // Keep parent open if sub-items match
        }

        if (item.child_menu) {
          filteredChildMenu = filterMenu(item.child_menu, isMatch);
          if (filteredChildMenu.length > 0) isMatch = true; // Keep parent open if child-items match
        }

        if (isMatch || parentMatch) {
          return {
            ...item,
            menu: filteredSubMenu,
            child_menu: filteredChildMenu,
          };
        }
        return null; // Remove items that don't match
      })
      .filter(Boolean);
  };
  ...
  // Auto-expand menus if there's an active search
  useEffect(() => {
    if (searchTerm) {
      const newOpenDropdowns = {};
      const setOpenMenus = (menu, indexPath = "") => {
        menu.forEach((item, index) => {
          const path = `${indexPath}${index}`;
          newOpenDropdowns[path] = true; // Keep expanded
          if (item.menu) setOpenMenus(item.menu, `${path}-`);
          if (item.child_menu) setOpenMenus(item.child_menu, `${path}-`);
        });
      };
      setOpenMenus(filteredMenu);
