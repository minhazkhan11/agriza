...
function ViewLead() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-lead") {
      navigate("/edit-lead", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      ...
      setBusinessCategory(response.data?.lead);
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/leads/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      toast.error("Lead is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/leads/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .catch((error) => {
        console.error("Error changed Lead status:", error);
        toast.error("Lead status is not changed");
      });
  };

  const rows = businessCategory.map((d) => ({
    id: d.id ? d.id : "N/A",
    code: d.id ? `LEAD${d.id}` : "N/A",
    business_name: d.business_name ? d.business_name : "N/A",
    year_of_establishment: d.year_of_establishment
      ? d.year_of_establishment
      : "N/A",
    name_of_dealing_person: d.name_of_dealing_person
      ? d.name_of_dealing_person
      : "N/A",
  ...
  const filteredRows = rows
    .filter((d) => {
      const isSearchMatch = d.business_name
        .toLowerCase()
  ...
  const columns = [
    ...
    {
      field: "name_of_dealing_person",
      headerName: "Dealing Person Name",
      headerClassName: "super-app-theme--header",
      headerAlign: "center", // Align the column header to center
      width: 200,
      flex: 1,
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-lead", cellValues.row.id);
                  }}
                >
