...
function EditLeadForm() {
  ...
  const fetchDataFromAPI = async (rowId) => {
    try {
      const apiUrl = `${process.env.REACT_APP_API_BASE_URL}/v1/admin/leads/${rowId}`;
      ...
      if (response.status === 200) {
        ...
        console.log("formDetailsformDetailsformDetails12", data);
        setFormDetails({
          lead_category_id: data.lead_category_id?.id || "",
          lead_subcategory_id: data.lead_subcategory_id?.id || "",
          is_bussiness: data.is_bussiness || "yes",
          gst_number: data.gst_number || "",
          pan_number: data.pan_number || "",
          type_of_organization: data.type_of_organization?.id || "",
          business_name: data.business_name || "",
          r_office_address: data.r_office_address || "",
          r_office_pincode_id: data.r_office_pincode_id?.id || "",
          r_office_place_id: data.r_office_place_id?.id || "",
          postal_office_address: data.postal_office_address || "",
          postal_office_pincode_id: data.postal_office_pincode_id?.id || "",
          postal_office_place_id: data.postal_office_place_id?.id || "",
          year_of_establishment: data.year_of_establishment || "",
          nearest_rack_point_id: data.nearest_rack_point_id || [],
          name_of_dealing_person: data.name_of_dealing_person || "",
          mobile_number: data.mobile_number || "",
          whatsapp_number: data.whatsapp_number || "",
          email: data.email || "",
          alternative_number: data.alternative_number || "",
          pesticide_license_number: data.pesticide_license_number || "",
          seed_license_number: data.seed_license_number || "",
          msme_udyam_registration_number:
            data.msme_udyam_registration_number || "",
          business_license_type: data.business_license_type || "",
          wholesaler_fertilizer_license_number:
            data.wholesaler_fertilizer_license_number || "",
          wholesaler_fms_id: data.wholesaler_fms_id || "",
          retailer_fertilizer_license_number:
            data.retailer_fertilizer_license_number || "",
          retailer_fms_id: data.retailer_fms_id || "",
        });
        setSelectedProductCategory(data.product_category_ids);
        setSelectedProductCategoryIds(
          data.product_category_ids.map((section) => section.id)
        );
        setSelectedProductSubCategory(data.product_sub_category_ids);
        setSelectedProductSubCategoryIds(
          data.product_sub_category_ids.map((section) => section.id)
        );
        setSelectedProductChildCategory(data.product_child_category_ids);
        setSelectedProductChildCategoryIds(
          data.product_child_category_ids.map((section) => section.id)
        );
        setSelectedDiscreetMarketer(data.discreet_marketer_id);
        setSelectedDiscreetMarketerIds(
          data.discreet_marketer_id.map((section) => section.id)
        );
      } else {
  ...
  const handleFormSubmit = () => {
   if (!String(formDetails.lead_category_id).trim()) {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/leads/update`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
  ...
  console.log("formDetailsformDetailsformDetails", formDetails);
...
{console.log('leadSubCategory' , leadSubCategory)}
                <Autocomplete
                  id="state-autocomplete"
            ...
            >
              ...
              >
              {console.log('formDetails.is_bussiness' , formDetails.is_bussiness)}
                <RadioGroup
