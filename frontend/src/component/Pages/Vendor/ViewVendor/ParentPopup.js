...
function ParentPopup({ popupDetail, rowData }) {
  ...
  const cartData = {rowData:rowData , type:'vendor'}
      ...
      >
        ...
        >
          {detail?.title === "New Payments" ? (
            <PaymentFrom detail={detail} handlePopUp={handlePopUp} rowData={rowData} />
          ) : (
            <ChildPopup detail={detail} popupDetail={popupDetail} handlePopUp={handlePopUp} rowData={rowData} />
          )}
        </Modal>
      </div>
    </div>
