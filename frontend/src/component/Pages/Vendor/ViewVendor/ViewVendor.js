...
function ViewVendor() {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/be_vendor/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      toast.error("vendor is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/be_vendor/update`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("vendor status changed successfully", response);
        fetchData();
        toast.success("vendor status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed vendor status:", error);
        toast.error("vendor status is not changed");
      });
  };
  ...
  const handleLogin = async (rowId) => {
    try {
      ...
      toast.error("Vendor is Inactive so cannot login");
