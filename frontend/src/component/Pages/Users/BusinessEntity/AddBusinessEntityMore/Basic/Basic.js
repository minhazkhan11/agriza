...
function Basic({ setValue, setBusinessEntityId, setconstitutionsName }) {
  ...
  const handleFormSubmit = async () => {
    console.log("setFormDetails123", formDetails);
    ...
    if (!formDetails.cin_gumasta.trim()) {
      toast.warn("Please enter a CIN or Gumasta Number dfbgfdbgf.");
      return;
    }
    ...
    const isAdminPresent = formDetails.owner_details.some(
      (person) => person.admin === "yes"
    );

    if (!isAdminPresent) {
      toast.warn("Please select at least one Owner/Partner/Director as Admin.");
      return;
    }
