...
function GstDetail({ setValue, businessEntityId }) {
  ...
  const handleFormSubmit = () => {
    for (let row of bankDetailsArray) {
      ...
      if (!row.gst_file) {
        toast.warn("Please upload a  GST Certificate (3 pager)");
        return;
      }

      if (row.persons.length === 0) {
        toast.warn(
          "Please enter at least one Key Managerial Person's Details."
        );
        return;
      }

      const isAdminPresent = row.persons.some(
        (person) => person.admin === "yes"
      );

      if (!isAdminPresent) {
        toast.warn(
          "Please select at least one Key Managerial Persons Details as Admin."
        );
        return;
      }
    }
    ...
    axios
      ...
      .catch((error) => {
        console.log("error124error124error124error124", error);
                ...
                >
                  ...
                  >
                    ...
                    <Autocomplete
                      ...
                      disableClearable
                      // value={place[index].find((sub) => sub.id === row.place_id)}
