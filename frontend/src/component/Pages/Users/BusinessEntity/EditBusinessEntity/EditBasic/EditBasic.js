...
function EditBasic({ setValue, setBusinessEntityId, setconstitutionsName }) {
  ...
  const handleFormSubmit = async () => {
    ...
    const data = {
      ...
      module_id: JSON.stringify(selectedModuleType),
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.log("responseobject123", response.data);
  ...
  const fetchModuleType = async () => {
    try {
      ...
      if (response.data && response.data.integrated_module_plans) {
        setModuleType(response.data.integrated_module_plans);
      } else {
        console.error("Invalid API response format:", response);
      }
    } catch (error) {
  ...
  console.log("selectedPostalPinId", selectedPostalPinId, postalPlace);
            ...
            >
              ...
              >
                ...
                <Autocomplete
                  ...
                  // value={postalPlace.find(
                  //   (sub) => sub.id === formDetails.postal_place_id
                  // )}
                  value={selectedPostalPlace}
