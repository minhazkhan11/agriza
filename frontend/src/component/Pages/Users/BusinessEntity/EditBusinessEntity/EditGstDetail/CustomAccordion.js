...
export default function CustomizedAccordions({
  ...
  const handleDeleteWithId = async (rowId, bankIndex, ownerIndex) => {
    try {
      const decryptedToken = decryptData(sessionStorage.getItem("token"));
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/business_entity_basic/parson/${rowId}`,
        {
          headers: {
      ...
      toast.success("Key Managerial Persons Details deleted successfully");
      setFormDetails((prev) =>
        prev.map((bank, i) =>
          i === bankIndex
            ? {
      ...
      toast.error("Key Managerial Persons Details is not deleted");
              ...
              >
                ...
                <div>
                  <Button
                    ...
                    variant={data?.admin === "yes" ? "contained" : "outlined"}
                    color={data?.admin === "yes" ? "primary" : "default"}
                  >
                    {data?.admin === "yes" ? "Manager" : "Make Manager"}
                  </Button>
                  <IconButton
                    onClick={(event) => {
                      event.stopPropagation();
                    ...
                    <LightTooltip title="Delete">
                      <DeleteIcon
                        ...
                        onClick={(event) => {
                          event.stopPropagation();
                          if (data?.id) {
                            handleDeleteWithId(data.id, mainindex, index);
                          } else {
                            handleDelete(mainindex, index);
                          }
                        }}
