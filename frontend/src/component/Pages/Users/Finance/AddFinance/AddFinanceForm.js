...
function AddBusinessEntityForm() {
  ...
  const handleCheck = () => {
    ...
    if (isBusiness === "registered") {
      if (!gstRegex.test(name)) {
        toast.warn("GST Number must be exactly 15  characters.");
        return;
      }
    }

    if (isBusiness === "notregistered") {
    ...
    axios
      ...
      .then((response) => {
        response.data.message === "Business Entity Already Registered"
          ? handlePopUp()
          : navigate("/create-lending", { state: data });
      })
      .catch((error) => {
        toast.error(error);
      });
