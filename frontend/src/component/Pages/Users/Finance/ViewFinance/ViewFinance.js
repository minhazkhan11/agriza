...
function ViewBusinessEntity() {
  ...
  const handleCheck = (type, rowId) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        navigate("/edit-lending", { state: localState });
      })
      .catch((error) => {
        toast.error(error);
      });
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/business_entity_basic/user_type/finance`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      toast.error("Finance is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      ...
      .then((response) => {
        console.log("Finance status changed successfully", response);
        fetchData();
        toast.success("Finance status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Finance status:", error);
        toast.error("Finance status is not changed");
      });
  };

  const handleCenterLogin = async (rowId) => {
    try {
      const decryptedToken = decryptData(sessionStorage.getItem("token"));
      const response = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/v1/client/centers/auths/signin/user_access/${rowId}`,
      ...
      toast.error("Finance is Inactive so cannot login");
          ...
          >
            ...
            >
              <Paper>
                <IconButton
                  onClick={() =>
                    handleCenterLogin(cellValues.row.center_user_id)
                  }
                >
                ...
                <IconButton
                  onClick={() => {
                    handleCheck("edit-lending", cellValues.row);
                  }}
                >
