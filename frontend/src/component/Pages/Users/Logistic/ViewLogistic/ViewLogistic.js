...
function ViewBusinessEntity() {
  ...
  const handleCheck = (type, rowId) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        navigate("/edit-logistics", { state: localState });
      })
      .catch((error) => {
        toast.error(error);
      });
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/business_entity_basic/user_type/logistics`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      toast.error("Logistic is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      ...
      .then((response) => {
        console.log("Logistic status changed successfully", response);
        fetchData();
        toast.success("Logistic status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Business Entity status:", error);
        toast.error("Logistic status is not changed");
      });
  };
  ...
  const handleCenterLogin = async (rowId) => {
    try {
      ...
      toast.error("Logistic is Inactive so cannot login");
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleCheck("edit-logistics", cellValues.row);
                  }}
                >
