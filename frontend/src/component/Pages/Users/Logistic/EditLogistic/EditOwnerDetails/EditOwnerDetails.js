  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        toast.success("Authorised Detail created successfully!");
        setTimeout(() => {
          setValue(2);
        }, 1000);
      } else {
        ...
        toast.error(
          `Authorised Detail is not created! ${response.data.message}`
        );
      }
    } catch (error) {
      // Handling errors
      if (
        ...
        toast.error(
          "An unexpected error occurred while creating the Authorised Detail."
        );
      }
