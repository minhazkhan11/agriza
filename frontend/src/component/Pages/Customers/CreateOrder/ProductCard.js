...
export default function ProductCard({ data,    userData, rowData }) {
  ...
  const updateLocalStorage = (qty, selectedUnit, productId) => {
    ...
    if (selectedUnit === data?.primary_unit_id?.unit_name) {
      unitPrice = (
        data?.selling_price / (data?.primary_quantity || 1)
      ).toFixed(2);
    } else if (selectedUnit === data?.secondary_unit_id?.unit_name) {
      unitPrice = (
        data?.selling_price /
        (data?.secondary_quantity || 1)
      ).toFixed(2);
    ...
    const newItem = {
      ...
      primary_unit_price: data?.selling_price,
      price: unitPrice,
      finalPrice: data?.selling_price,
    ...
    localStorage.setItem("cartstate", JSON.stringify(cartData));
    sessionStorage.setItem("cartstate", JSON.stringify(cartData));
    localStorage.setItem("customerDetails", JSON.stringify(rowData));
    sessionStorage.setItem("customerDetails", JSON.stringify(rowData));
    ...
    <Card className={classes.root}>
      ...
      <CardContent>
        ...
        <div className={`${customclasses.dflex}`}>
          <Typography
            className={`${customclasses.fw600} ${customclasses.mt0_5} ${customclasses.textdecorationline}`}
            variant="body2"
            color="textSecondary"
          >
            {data?.cross_price} /{" "}
          ...
          <Typography
            className={`${customclasses.fw600} ${customclasses.ml0_5} ${customclasses.mt0_5}`}
            variant="body2"
          >
            {data?.selling_price} /{" "}
            {data?.primary_unit_id?.unit_name}
        ...
        <Typography
          className={`${customclasses.fontSize} ${customclasses.mt0_5}`}
          color="textSecondary"
        >
          {" "}
          ({data?.gst_name} incl.)
