...
function ViewCustomerActivity() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-Activity") {
      navigate("/edit-Activity", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/order_activity/customer/data`,
        {
          headers: {
      ...
      setActivityOrder(response.data.order_activity);
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/order_activity/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      const updateddatatata = ActivityOrder.filter((data) => data.id !== rowId);
      setActivityOrder(updateddatatata);
    } catch (error) {
      toast.error("Activity is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/order_activity/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        fetchData();
        toast.success("Activity status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Activity status:", error);
        toast.error("Activity status is not changed");
      });
  };

  const rows = ActivityOrder.map((d) => ({
    id: d.id || "N/A",
    code: d.id ? `Activity${d.id}` : "N/A",
    type: d.type || "N/A",
    afollow_up_date: d.afollow_up_date || "N/A",
    Comments: d.Comments || "N/A",
    follow_up_date: d.follow_up_date
      ? (() => {
        const date = new Date(d.follow_up_date);
          ...
          >
            ...
            >
              <Paper>
                ...
                {/* <IconButton
                  onClick={() => {
                    handleButtonClick("edit-Activity", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search Activity name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
...
export default ViewCustomerActivity;
