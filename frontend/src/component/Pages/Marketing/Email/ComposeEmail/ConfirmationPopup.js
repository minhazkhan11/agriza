    ...
    <>
      <Fade in={open}>
        ...
        >
          ...
          <div
            className={`${classes.pagescroll} ${classes.maxh75} ${classes.mt1} ${classes.positionrelative}`}
          >
            <div>
              {!selectedCourse && selectedLearner.length === 0 && (
                <Typography>
                  Are you want to send Mail to all learners?
                </Typography>
              )}
              ...
              {selectedLearner.length === 0 && selectedCourse && (
                <>
                  <Typography className={classes.mb1}>
                    Are you want to send Mail to selected learners?
                  </Typography>
                  {learners.map((data, index) => (
                    <Typography key={index}>
                      {`${index + 1}) ${data.full_name} - ${data.email}`}
                    </Typography>
                  ))}
                </>
              )}
