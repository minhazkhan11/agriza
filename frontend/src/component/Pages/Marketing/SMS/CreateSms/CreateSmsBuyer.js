...
function CreateSmsBuyer() {
  ...
  const [selectedLearnerPhone, setSelectedLearnerPhone] = useState([]);
  ...
  const handleLearnerChange = (event, value) => {
    ...
    setSelectedLearnerPhone(value.map((learner) => learner.phone));
  };
      ...
      text: {
        ...
        sender_name:
          selectedLearner.length === 0
            ? learners.map((learner) => learner.full_name)
            : selectedLearner.map((learner) => learner.full_name),
        phone:
          selectedLearner.length === 0
            ? learners.map((learner) => learner.phone)
            : selectedLearner.map((learner) => learner.phone),
        email:
          selectedLearner.length === 0
            ? learners.map((learner) => learner.email)
            : selectedLearner.map((learner) => learner.email),
      },
    };
  ...
  ${classes.lineheight}`}
            ...
            >
              ...
              >
                ...
                >
                  Enter Buyer Number or leave blank for all
                </FormLabel>
                ...
                <Autocomplete
                  ...
                  value={learners.filter((learner) =>
                    selectedLearnerPhone.includes(learner.phone)
                  )}
