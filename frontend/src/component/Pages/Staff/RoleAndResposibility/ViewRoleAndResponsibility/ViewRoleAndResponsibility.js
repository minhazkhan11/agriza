...
function ViewProductCategory() {
  ...
  const handleButtonClick = (type, rowId, rowName, isEdit) => {
    const data = {
      rowId: rowId,
      rowName: rowName,
      isEdit: isEdit,
  ...
  const fetchData = async () => {
    try {
      ...
      setProductCategory(response.data.menu_plan);
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/menu_plan/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      fetchData();
      handleClose();
      toast.success("Roles & Responsbility deleted successfully");
      const updateddatatata = productCategory.filter(
        (data) => data.id !== rowId
      );
      setProductCategory(updateddatatata);
    } catch (error) {
      toast.error("Roles & Responsbility is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/menu_plan/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log(
          "Roles & Responsbility status changed successfully",
          response
        );
        fetchData();
        toast.success("Roles & Responsbility status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Roles & Responsbility status:", error);
        toast.error("Roles & Responsbility status is not changed");
      });
  };
  const rows = productCategory.map((d) => ({
    id: d.id ? d.id : "N/A",
    category_name: d.menu_name ? d.menu_name : "N/A",
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick(
                     ...
                     (decryptedUserRole !== "superadmin" && cellValues.row.srno > 5) || decryptedUserRole === "superadmin"
                    );
                  }}
                >
                  <LightTooltip title="Add Permission">
                    <AddIcon />
                  </LightTooltip>
                </IconButton>
                {(decryptedUserRole !== "superadmin" &&
                  cellValues.row.srno > 5) ||
                decryptedUserRole === "superadmin" ? (
                  <>
                    ...
                    <IconButton
                      onClick={() => {
                        handleButtonClick("edit-roles", cellValues.row.id);
                      }}
                    >
