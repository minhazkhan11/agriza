...
function AddOfficeStaffForm() {
  ...
  const handleFormSubmit = async () => {
    ...
    // if (decryptedUserRole !== "superadmin" || decryptedUserRole !== "user") {
    //   if (!String(selectedBusinessArea).trim()) {
    //     toast.warn("Please Select a Business Area.");
    //     return;
    //   }
    // }
    // if (decryptedUserRole !== "superadmin") {
    //   if (!String(selectedBusinessAreaId).trim()) {
    //     toast.warn(`Please Select a ${selectedBusinessArea}.`);
    //     return;
    //   }
    // }
    // if (decryptedUserRole !== "superadmin") {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/staff/add`,
        formData,
        {
          headers: {
      ...
      if (response.status === 200) {
        ...
        toast.error(`Staff is not created! ${response.data.message}`);
      }
    } catch (error) {
      // Handling errors
      if (
        ...
        toast.error("An unexpected error occurred while creating the Staff.");
      }
        ...
        <FormControl className={`${classes.w100}`}>
          ...
          >
            ...
            >
              ...
              {console.log("aadhar_upload", files)}
              <div className={`${classes.w24} ${classes.mt1_5}`}>
                <UploadPreview
                  thumbnailImagePreview={
                    files.pan_upload && URL.createObjectURL(files.pan_upload)
                  }
                />
              </div>
            </div>
...
export default AddOfficeStaffForm;
