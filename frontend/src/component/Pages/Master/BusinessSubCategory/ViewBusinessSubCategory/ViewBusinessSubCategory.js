...
function ViewBusinessSubCategory() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-business-sub-category") {
      navigate("/edit-business-sub-category", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/sub_category`,
        {
          headers: {
      ...
      setBusinessSubCategory(response.data.sub_category);
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/sub_category/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      fetchData();
      handleClose();
      toast.success("Business Sub Category deleted successfully");
      const updateddatatata = businessSubCategory.filter((data) => data.id !== rowId);
      setBusinessSubCategory(updateddatatata);
    } catch (error) {
      toast.error("Business Sub Category is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/sub_category/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        fetchData();
        toast.success("Business Sub Category status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Business Sub Category status:", error);
        toast.error("Business Sub Category status is not changed");
      });
  };

  const rows = businessSubCategory.map((d) => ({
    id: d.id?d.id:"N/A",
    sub_category_name: d.sub_category_name?d.sub_category_name:"N/A",
    category_name: d.business_category_id?d.business_category_id.category_name:"N/A",
  ...
  const columns = [
    ...
    },
    {
      ...
      headerName: "Business Sub Category ",
      headerClassName: "super-app-theme--header",
           ...
           {isActive ? (
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-business-sub-category", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search business sub category name",
      inputlable: "yes",
    },
  ];
  return (
...
export default ViewBusinessSubCategory;
