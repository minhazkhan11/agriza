...
function EditShipToPartyForm() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/ship_to_party/${rowId}`,
        {
          headers: {
      ...
      const res = response.data.ship_to_party;
      setSelectedPin(res.pincode_id.id);
      setSelectedCustomerId(res.added_by);
      const selected = customer?.find((c) => c.id === res.gst_id.id);
      setSelectedCustomerId(selected);
      setSelectedGstId(res.gst_id.id);
      setFormDetails({
        id: rowId,
      ...
      setLicenseDetails(formateData(res.license_info));
    } catch (error) {
  ...
  const handleFormSubmit = () => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/ship_to_party/update`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
            ...
            >
              ...
              >
                ...
                <Autocomplete
                  ...
                  value={customer.find((sub) => sub.id === selectedGstId) || ""}
                ...
                >
                  ...
                  >
                    ...
                    <Autocomplete
                      ...
                      // )}
                      value={licenseDetails[index].licenseType}
                      onChange={(event, newValue) =>
...
export default EditShipToPartyForm;
