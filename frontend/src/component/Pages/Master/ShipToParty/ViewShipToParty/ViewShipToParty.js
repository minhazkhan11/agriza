...
function ViewShipToParty() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-ship-to-party") {
      navigate("/edit-ship-to-party", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/ship_to_party`,
        {
          headers: {
      ...
      setWareHouse(response.data.ship_to_party);
      console.log(response.data.ship_to_party);
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    ...
    try {
      ...
      fetchData();
      handleClose();
      toast.success("Ship To Party deleted successfully");
      const updateddatatata = wareHouse.filter((data) => data.id !== rowId);
      setWareHouse(updateddatatata);
    } catch (error) {
      toast.error("Ship To Party is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/ship_to_party/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Ship To Party status changed successfully", response);
        fetchData();
        toast.success("Ship To Party status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Ship To Party status:", error);
        toast.error("Ship To Party status is not changed");
      });
  };

  const rows = wareHouse.map((d) => ({
    id: d.id ? d.id : "N/A",
    code: d.id ? `STP${d.id}` : "N/A",
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-ship-to-party", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search Ship To Party Name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
...
export default ViewShipToParty;
