...
function ViewLicenseCategory() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-license-category") {
      navigate("/edit-license-category", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/license_category`,
        {
          headers: {
      ...
      setBusinessCategory(response.data.license_category);
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/license_category/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      toast.error("License Category is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/license_category/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("License Category status changed successfully", response);
        fetchData();
        toast.success("License Category status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Business Segment status:", error);
        toast.error("License Category status is not changed");
      });
  };

  const rows = businessCategory.map((d) => ({
    id: d.id?d.id:"N/A",
    category_name: d.license_category_name?d.license_category_name:"N/A",
  ...
  const columns = [
    ...
    {
      field: "category_name",
      headerName: "License Category Name",
      headerClassName: "super-app-theme--header",
           ...
           {isActive ? (
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-license-category", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search license category name",
      inputlable: "yes",
    },
  ];
  return (
...
export default ViewLicenseCategory;
