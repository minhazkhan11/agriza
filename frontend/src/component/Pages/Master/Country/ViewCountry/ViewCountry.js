...
function ViewCountry() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-country") {
      navigate("/edit-country", { state: rowId });
    }
  };
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/country/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      const updateddatatata = country.filter((data) => data.id !== rowId);
      setCountry(updateddatatata);
    } catch (error) {
      toast.error("Country is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/country/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        fetchData();
        toast.success("Country status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Country status:", error);
        toast.error("Country status is not changed");
      });
  };

  const rows = country.map((d) => ({
    id: d.id?d.id:"N/A",
    category_name: d.country_name?d.country_name:"N/A",
    country_code: d.country_code?d.country_code:"N/A",
           ...
           {isActive ? (
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-country", cellValues.row.id);
                  }}
                >
