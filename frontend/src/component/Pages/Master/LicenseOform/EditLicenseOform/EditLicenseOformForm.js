...
function EditLicenseOformForm() {
  ...
  const fetchData = async () => {
    try {
      ...
      const data = response.data.o_form_versioning;
      // Set basic form details
      setFormDetails({
        id: rowId,
        o_form_versioning_name: data.o_form_versioning_name || "",
        license_id: data.license_id.id || "",
      });
      setLicenseProductIds(data?.license_products.map((product) => product.id));
    } catch (error) {
  ...
  const handleFormSubmit = () => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/o_form_versioning/update`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      >
        <FormControl className={`${classes.w100}`}>
          ...
          >
            ...
            {formDetails.license_id && licenseProductIds && (
              <div
...
export default EditLicenseOformForm;
