...
function ViewProductApproval() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-single-product") {
      navigate("/edit-single-product", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/master_product/auth/pending`,
        {
          headers: {
      ...
      console.log(response.data.master_product, "new product");
  ...
  const rows = product.map((d) => ({
    id: d.id ? d.id : "N/A",
    code: d.id ? `PROD${d.id}` : "N/A",
    category_name: d.product_name ? d.product_name : "N/A",
    product_class_name: d.is_license ? d.is_license : "N/A",
    hsn_code: d.hsn_code ? d.hsn_code : "N/A",
    gst_applicable: d.gst_applicable ? d.gst_applicable : "N/A",
    gst_percent: d.gst_id ? d.gst_id.gst_name : "N/A",
    product_child_category: d.product_child_category_id
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-single-product", cellValues.row.id);
                  }}
                >
...
export default ViewProductApproval;
