...
function ViewRegion() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-region") {
      navigate("/edit-region", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      ...
      setRegionCategory(response.data.regionArray);
      // console.log('rr',response.data.regionArray)
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const updateddatatata = regionCategory.filter(
        (data) => data.id !== rowId
      );
      setRegionCategory(updateddatatata);
    } catch (error) {
      toast.error("Region is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/business_area_region/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Region status changed successfully", response);
        fetchData();
        toast.success("Region status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Region status:", error);
        toast.error("Region status is not changed");
      });
  };

  const rows = regionCategory.map((d) => ({
    id: d.id ? d.id : "N/A",
    code: d.id ? `REGI${d.id}` : "N/A",
    name: d.name ? d.name : "N/A",
    area: d.areaData ? d.areaData.map((t)=>(t.name))?.join(', ') : "N/A",
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-region", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search region name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
