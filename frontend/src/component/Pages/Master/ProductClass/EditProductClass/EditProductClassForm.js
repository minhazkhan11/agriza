...
function EditProductClassForm() {
  ...
  const fetchDataFromAPI = async (rowId) => {
    try {
      const apiUrl = `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_class/${rowId}`;
      ...
      if (response.status === 200) {
        const data = response.data.productClass;
  ...
  const handleFormSubmit = () => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_class/update`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .catch((error) => {
        ...
        toast.error("Product Class is not updated");
      });
  };
...
export default EditProductClassForm;
