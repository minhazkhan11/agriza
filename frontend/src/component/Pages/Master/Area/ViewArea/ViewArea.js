...
function ViewArea() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-area") {
      navigate("/edit-area", { state: rowId });
    }
  };
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const updateddatatata = area.filter((data) => data.id !== rowId);
      setArea(updateddatatata);
    } catch (error) {
      toast.error("Area is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/business_area/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        fetchData();
        toast.success("Area status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Area status:", error);
        toast.error("Area status is not changed");
      });
  };

  const rows = area.map((d) => ({
    id: d.id ? d.id : "N/A",
    code: d.id ? `AREA${d.id}` : "N/A",
    name: d.name ? d.name : "N/A",
    territory: d.teritariData
      ? d.teritariData.map((t) => t.name)?.join(", ")
      : "N/A",
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-area", cellValues.row.id);
                  }}
                >
