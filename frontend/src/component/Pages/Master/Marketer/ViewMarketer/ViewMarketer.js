...
function ViewMarketer() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-marketer") {
      navigate("/edit-marketer", { state: rowId });
    }
  };
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/marketers/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      const updateddatatata = marketer.filter((data) => data.id !== rowId);
      setMarketer(updateddatatata);
    } catch (error) {
      toast.error("Marketer is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/marketers/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Marketer status changed successfully", response);
        fetchData();
        toast.success("Marketer status changed successfully");
        handleClose();
      })
      .catch((error) => {
  ...
  const rows = marketer.map((d) => ({
    id: d.id ? d.id : "N/A",
    category_name: d.marketer_name ? d.marketer_name : "N/A",
    description: d.description ? d.description : "N/A",
    alias_name: d.alias_name ? d.alias_name : "N/A",
  ...
  const columns = [
    ...
    {
      field: "category_name",
      headerName: "Marketer Legal Name",
      headerClassName: "super-app-theme--header",
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-marketer", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search Marketer name",
      inputlable: "yes",
    },
  ];
  return (
