...
function ViewZone() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-zone") {
      navigate("/edit-zone", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      ...
      setZoneCategory(response.data.business_area_zone);
      // console.log('pp',response.data.business_area_zone)
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const updateddatatata = zoneCategory.filter((data) => data.id !== rowId);
      setZoneCategory(updateddatatata);
    } catch (error) {
      toast.error("Zone is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/business_area_zone/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Zone status changed successfully", response);
        fetchData();
        toast.success("Zone status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Zone status:", error);
        toast.error("Zone status is not changed");
      });
  };
  // regionData
  const rows = zoneCategory.map((d) => ({
    id: d.id ? d.id : "N/A",
    code: d.id ? `ZONE${d.id}` : "N/A",
    name: d.name ? d.name : "N/A",
    region: d.regionData ? d.regionData.map((t)=>(t.name))?.join(', ') : "N/A",
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-zone", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search zone name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
