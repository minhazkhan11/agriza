...
function ViewDistrict() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-district") {
      navigate("/edit-district", { state: rowId });
    }
  };
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/district/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      const updateddatatata = district.filter((data) => data.id !== rowId);
      setDistrict(updateddatatata);
    } catch (error) {
      toast.error("District is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/district/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("District status changed successfully", response);
        fetchData();
        toast.success("District status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed District status:", error);
        toast.error("District status is not changed");
      });
  };

  const rows = district.map((d) => ({
    id: d.id?d.id:"N/A",
    category_name: d.district_name?d.district_name:"N/A",
    country_code: d.country_name?d.country_name:"N/A",
    state_code: d.state_name?d.state_name:"N/A",
           ...
           {isActive ? (
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-district", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search District name",
      inputlable: "yes",
    },
  ];
  return (
