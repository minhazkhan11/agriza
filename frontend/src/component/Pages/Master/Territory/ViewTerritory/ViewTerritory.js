...
function ViewTerritory() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-territory") {
      navigate("/edit-territory", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      ...
      console.log('f',response?.data?.product_areas
      )
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const updateddatatata =  territory.filter(
        (data) => data.id !== rowId
      );
      setTerritory(updateddatatata);
    } catch (error) {
      toast.error("Territory is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/business_area_teritari/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Territory  status changed successfully", response);
        fetchData();
        toast.success("Territory  status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Territory status:", error);
        toast.error("Territory status is not changed");
      });
  };
  ...
  const rows =  territory?.map((d) => ({
    id: d.id ? d.id : "N/A",
    code: d.id ? `TERR${d.id}` : "N/A",
    name: d.name ? d.name : "N/A",
    demographic_include: d.demographic_include ?  getIncludeData(d.demographic_include,d)?.join(", ") : "N/A",
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-territory", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search territory name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
