...
function EditTerritoryForm() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/business_area_teritari/${rowId}`,
        {
          headers: {
      ...
      const territory = response.data.product_area;
      console.log("territoryad", territory.name);
      ...
      setFormDetails({
        ...
        demographic_include_id: territory.demographic_include_id,
        demographic_exclude_id: territory.demographic_exclude_id,
        demographic_include: territory.demographic_include,
        demographic_exclude: territory.demographic_exclude,
        demographic_exclude_2: territory.demographic_exclude_2,
        demographic_exclude_2_id: territory.demographic_excludes_id_2,
      });

      setShowDemographicExclude2(
        territory.demographic_exclude_2 ? true : false
      );

      // Set demographic includes/excludes
      setSelectedDemographicInclude({ title: territory.demographic_include });
      setSelectedDemographicExclude({ title: territory.demographic_exclude });
      setSelectedDemographicExclude2({
        title: territory.demographic_exclude_2,
      });

      // Common demographic selection
      const demographicData = {
        demographicInclude: Array.isArray(territory?.demographic_include_id)
          ? territory.demographic_include_id
          : [],
        demographicExclude: Array.isArray(territory?.demographic_exclude_id)
          ? territory.demographic_exclude_id
          : [],
        demographicExclude2: Array.isArray(territory?.demographic_exclude_2_id)
          ? territory.demographic_exclude_2_id
          : [],
      };
  ...
  const handleFormSubmit = () => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/business_area_teritari/update`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        toast.success("territory Information Updated Successfully");
            ...
            >
              ...
              <div
                className={`${classes.dflex} ${classes.flexdirectioncolumn}  ${
                  showDemographicExclude2 ? classes.w24_5 : classes.w20
                }`}
              ></div>
            </div>
            {showDemographicExclude2 && (
              ...
              >
                ...
                <div
                  className={`${classes.dflex} ${classes.flexdirectioncolumn}  ${classes.w20}`}
                ></div>
              </div>
            )}
          </div>
