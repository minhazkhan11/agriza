...
function EditBusinessCategoryForm() {
  ...
  const handleClose = () => {
    navigate("/business-segment-list");
  };

  const fetchDataFromAPI = async (rowId) => {
    try {
      const apiUrl = `${process.env.REACT_APP_API_BASE_URL}/v1/admin/category/${rowId}`;
  ...
  const handleFormSubmit = () => {
    ...
    axios
      .put(`${process.env.REACT_APP_API_BASE_URL}/v1/admin/category/update`, data, {
        headers: {
          Authorization: `Bearer ${decryptedToken}`,
        },
      })
      ...
      .catch((error) => {
        ...
        toast.error("Business Segment is not updated");
      });
  };
