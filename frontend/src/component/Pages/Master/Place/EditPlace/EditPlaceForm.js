...
function EditPlaceForm() {
  ...
  const fetchDataFromAPI = async (rowId) => {
    try {
      const apiUrl = `${process.env.REACT_APP_API_BASE_URL}/v1/admin/place/${rowId}`;
  ...
  const handleFormSubmit = () => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/place/update`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .catch((error) => {
        ...
        toast.error("Place is not updated");
      });
  };
