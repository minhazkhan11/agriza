...
function ViewMarketerApproval() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-single-marketer") {
      navigate("/edit-single-marketer", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/marketers/auth/pending`,
        {
          headers: {
      ...
      console.log(response.data.marketers, "new product");
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-single-marketer", cellValues.row.id);
                  }}
                >
...
export default ViewMarketerApproval;
