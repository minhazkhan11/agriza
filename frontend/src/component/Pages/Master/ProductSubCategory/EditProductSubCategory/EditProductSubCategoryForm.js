...
function EditProductSubCategoryForm() {
  ...
  const handleClose = () => {
    navigate("/product-sub-category-list");
  };

  const fetchDataFromAPI = async (rowId) => {
    try {
      const apiUrl = `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_sub_category/${rowId}`;
      ...
      if (response.status === 200) {
        const data = response.data.product_sub_category;
        setThumbnailImage(data?.sub_category_image);
        setThumbnailImagePreview(data?.sub_category_image);
        setName(data.product_sub_category_name);
        setDescription(data.description);
        setSelectedProductCategory(data.Product_category_id);
        setSelectedProductCategoryId(data.Product_category_id.id);
      } else {
  ...
  const handleFormSubmit = async () => {
    ...
    if (!name.trim()) {
      toast.warn("Please enter Product Sub Category.");
      return;
    }
    if (!selectedBatchProductCategory.trim()) {
      toast.warn("Please Select Product Category");
    }
    const data = {
    ...
    try {
      ...
      formData.append("product_sub_category", JSON.stringify(productData));
      formData.append("sub_category_image", thumbnailImage);

      const response = await axios.put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_sub_category/update`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      if (response.status === 200) {
        ...
        toast.error(
          `Product Sub Category is not updated! ${response.data.message}`
        );
      }
    } catch (error) {
      // Handling errors
      if (
        ...
        toast.error(
          "An unexpected error occurred while creating the Product Sub Category"
        );
      }
...
export default EditProductSubCategoryForm;
