...
import AddProductCategoryFormPopUp from "../../ProductCategory/AddProductCategory/AddProductCategory";
import UploadPreview from "../../../../CustomComponent/UploadPreview";

function AddProductSubCategoryForm({ style }) {
  ...
  const handleFormSubmit = async () => {
    ...
    if (!name.trim()) {
      toast.warn("Please Enter Product Sub Category Name.");
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_sub_category/add`,
        formData,
        {
          headers: {
      ...
      if (response.status === 200) {
        toast.success("Product Sub Category Created Successfully");
        setTimeout(() => {
          style?.isPopUp
            ? navigate("/create-product-child-category")
        ...
        toast.error(
          `Product Sub Category is not created! ${response.data.message}`
        );
      }
    } catch (error) {
      // Handling errors
  ...
  return (
    <>
      ...
      >
        ...
        >
          <AddProductCategoryFormPopUp style={Heading} />
        </Modal>
      </div>
    </>
  );
}
export default AddProductSubCategoryForm;
