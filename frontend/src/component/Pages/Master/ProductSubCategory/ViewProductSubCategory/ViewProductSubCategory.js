...
function ViewProductSubCategory() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-product-sub-category") {
      navigate("/edit-product-sub-category", { state: rowId });
    }
  };
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_sub_category/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      const updateddatatata = productSubCategory.filter((data) => data.id !== rowId);
      setProductSubCategory(updateddatatata);
    } catch (error) {
      toast.error("Product Sub Category is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_sub_category/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Product Sub Category status changed successfully", response);
        fetchData();
        toast.success("Product Sub Category status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Product Sub Category status:", error);
        toast.error("Product Sub Category status is not changed");
      });
  };
  const rows = productSubCategory.map((d) => ({
    id: d.id?d.id:"N/A",
    code: d.id ? `PSC${d.id}` : "N/A",
    category_name: d.product_sub_category_name?d.product_sub_category_name:"N/A",
    product_class_name: d.Product_category_id.category_name?d.Product_category_id.category_name:"N/A",
  ...
  const columns = [
    ...
    {
      field: "category_name",
      headerName: "Product Sub Category Name",
      headerClassName: "super-app-theme--header",
           ...
           {isActive ? (
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-product-sub-category", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search product sub category name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
