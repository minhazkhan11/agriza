...
function ViewRackPoint() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-rack-point") {
      navigate("/edit-rack-point", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      ...
      setPlace(response.data.rakepoints);
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/rack_point/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      fetchData();
      handleClose();
      toast.success("Rack Point deleted successfully");
      const updateddatatata = place.filter((data) => data.id !== rowId);
      setPlace(updateddatatata);
    } catch (error) {
      toast.error("Rack Point is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/rack_point/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Rack Point status changed successfully", response);
        fetchData();
        toast.success("Rack Point status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Rack Point status:", error);
        toast.error("Rack Point status is not changed");
      });
  };

  const rows = place.map((d) => ({
    id: d.id?d.id:"N/A",
    category_name: d.rack_point?d.rack_point:"N/A",
    rack_point_distanse: d.rack_point_distanse?d.rack_point_distanse:"N/A",
    place_name: d.place?d.place.place_name:"N/A",
    country_code: d.place?d.place.pin_code:"N/A",
    tehsil_name: d.place ? d.place.tehsil_name : "N/A",
    district_name: d.place ? d.place.district_name : "N/A",
    state_name: d.place ? d.place.state_name : "N/A",
    country_name: d.place ? d.place.country_name : "N/A",
  ...
  const columns = [
    ...
    {
      field: "rack_point_distanse",
      headerName: "Rack Point Distanse",
      headerClassName: "super-app-theme--header",
      headerAlign: "center", 
      width: 200,
      sortable: true,
           ...
           {isActive ? (
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-rack-point", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search Rake Point Name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
