...
function ViewSupplierType() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-supplier-type") {
      navigate("/edit-supplier-type", { state: rowId });
    }
  };
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/supplier/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      const updateddatatata = supplierType.filter((data) => data.id !== rowId);
      setSupplierType(updateddatatata);
    } catch (error) {
      toast.error("Supplier Type is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/supplier/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Supplier Type status changed successfully", response);
        fetchData();
        toast.success("Supplier Type status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Supplier Type status:", error);
        toast.error("Supplier Type status is not changed");
      });
  };
  const rows = supplierType.map((d) => ({
    id: d.id?d.id:"N/A",
    category_name: d.supplier_name?d.supplier_name:"N/A",
           ...
           {isActive ? (
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-supplier-type", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search supplier type name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
...
export default ViewSupplierType;
