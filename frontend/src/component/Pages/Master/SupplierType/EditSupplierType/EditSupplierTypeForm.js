...
function EditSupplierTypeForm() {
  ...
  const fetchDataFromAPI = async (rowId) => {
    try {
      const apiUrl = `${process.env.REACT_APP_API_BASE_URL}/v1/admin/supplier/${rowId}`;
  ...
  const handleFormSubmit = () => {
    ...
    axios
      .put(`${process.env.REACT_APP_API_BASE_URL}/v1/admin/supplier/update`, data, {
        headers: {
          Authorization: `Bearer ${decryptedToken}`,
        },
      })
      ...
      .catch((error) => {
        ...
        toast.error("Supplier Type is not updated");
      });
  };
...
export default EditSupplierTypeForm;
