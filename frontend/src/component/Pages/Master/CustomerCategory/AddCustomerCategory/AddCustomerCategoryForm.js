...
function AddCustomerCategoryForm({ style }) {
  ...
  const handleClose = () => {
    style?.isPopUp ? style?.onClose() : navigate("/customer-category-list");
  };

  const handleFormSubmit = () => {

    if (!name.trim()) {
      toast.warn("Please enter Customer category name.");
      return;
    }
    ...
    axios
    .post(
      `${process.env.REACT_APP_API_BASE_URL}/v1/admin/customer_category/add`,
        data,
        {
          headers: {
...
export default AddCustomerCategoryForm;
