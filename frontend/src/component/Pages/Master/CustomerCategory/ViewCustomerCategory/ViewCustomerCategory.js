...
function ViewCustomerCategory() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-customer-category") {
      navigate("/edit-customer-category", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/customer_category`,
        {
          headers: {
      ...
      setCustomerCategory(response.data.customer_category);
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/customer_category/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      const updateddatatata = customerCategory.filter((data) => data.id !== rowId);
      setCustomerCategory(updateddatatata);
    } catch (error) {
      toast.error("Customer Category is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/customer_category/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        fetchData();
        toast.success("Customer Category status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Customer Category status:", error);
        toast.error("Customer Category status is not changed");
      });
  };
  const rows = customerCategory.map((d) => ({
    id: d.id ? d.id : "N/A",
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-customer-category", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search customer category name",
      inputlable: "yes",
    },
  ];
  return (
