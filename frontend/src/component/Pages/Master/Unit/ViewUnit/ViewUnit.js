...
function ViewUnit() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-unit") {
      navigate("/edit-unit", { state: rowId });
    }
  };
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/units/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      const updateddatatata = unit.filter((data) => data.id !== rowId);
      setUnit(updateddatatata);
    } catch (error) {
      toast.error("Unit is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/units/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Unit status changed successfully", response);
        fetchData();
        toast.success("Unit status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Unit status:", error);
        toast.error("Unit status is not changed");
      });
  };

  const rows = unit.map((d) => ({
    id: d.id ? d.id : "N/A",
    category_name: d.unit_name ? d.unit_name : "N/A",
    formal_name: d.formal_name ? d.formal_name : "N/A",
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-unit", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search unit name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
