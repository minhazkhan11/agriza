...
function ViewProductMaster() {
  ...
  const fetchData = async () => {
    try {
      ...
      setProductCategory(response.data.master_product);
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/master_product/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/master_product/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
  ...
  const rows = productCategory.map((d) => ({
    ...
    product_child_category: d.product_child_category_id ? d.product_child_category_id.product_child_category_name : "N/A",
...
export default ViewProductMaster;
