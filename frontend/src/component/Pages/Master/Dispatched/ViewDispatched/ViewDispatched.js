...
function ViewDispatched() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/order_so_po/dispatch/get_all/so`,
        {
          headers: {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (cellValues) => {
        ...
        if (["rejected", "delivered"].includes(status)) {
          return (
            <div
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search Dispatched Order name",
      inputlable: "yes",
    },
  ];
  return (
