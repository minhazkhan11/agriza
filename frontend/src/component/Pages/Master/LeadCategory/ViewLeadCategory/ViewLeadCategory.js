...
function ViewLeadCategory() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-lead-category") {
      navigate("/edit-lead-category", { state: rowId });
    }
  };
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/lead_category/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      fetchData();
      handleClose();
      toast.success("Lead Category deleted successfully");
      const updateddatatata = LeadCategory.filter((data) => data.id !== rowId);
      setLeadCategory(updateddatatata);
    } catch (error) {
      toast.error("Lead Category is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/lead_category/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Lead Category status changed successfully", response);
        fetchData();
        toast.success("Lead Category status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Lead Category status:", error);
        toast.error("Lead Category status is not changed");
      });
  };
  const rows = LeadCategory.map((d) => ({
    id: d.id ? d.id : "N/A",
    code: d.id ? `LC${d.id}` : "N/A",
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-lead-category", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search Lead category name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
