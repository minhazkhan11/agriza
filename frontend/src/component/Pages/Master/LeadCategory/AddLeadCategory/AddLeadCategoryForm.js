...
function AddLeadCategoryForm({ style }) {
  ...
  const handleClose = () => {
    style?.isPopUp ? style?.onClose() : navigate("/lead-category-list");
  };

  const handleFormSubmit = () => {

    if (!name.trim()) {
      toast.warn("Please enter lead category name.");
      return;
    }
    ...
    axios
    .post(
      `${process.env.REACT_APP_API_BASE_URL}/v1/admin/lead_category/add`,
        data,
        {
          headers: {
      ...
      .then((response) => {
        ...
        setTimeout(() => {
          style?.isPopUp
            ? navigate("/create-lead-sub-category")
            : navigate("/lead-category-list");
