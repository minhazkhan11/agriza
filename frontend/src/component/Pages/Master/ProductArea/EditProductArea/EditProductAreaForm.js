...
function EditProductAreaForm() {
  ...
  const handleDemographicId = (filedName, newValue) => {
    ...
    if (filedName === "demographicExclude") {
      setFormDetails((pre) => ({
        ...pre,
        demographic_excludes_id: newValue.map((subject) => subject.id),
      }));
    }
    if (filedName === "demographicExclude2") {
      setFormDetails((pre) => ({
  ...
  const fetchData = async () => {
    try {
      ...
      console.log("ProductCategory", response.data.product_area);
      const productArea = response.data.product_area;

      console.log("rrrr", productArea);

      // Set basic form details
      setFormDetails({
        id: rowId,
        name: productArea.name || "",
        short_name: productArea.short_name || "",
        code: productArea.code || null,
        marketer_id: productArea.marketer.id,
        demographic_includes_id: productArea.demographic_includes_id,
        demographic_excludes_id: productArea.demographic_excludes_id,
        demographic_include: productArea.demographic_include,
        demographic_exclude: productArea.demographic_exclude,
        demographic_exclude_2: productArea.demographic_exclude_2,
        demographic_exclude_2_id: productArea.demographic_excludes_id_2,
      });

      setShowDemographicExclude2(
        productArea.demographic_exclude_2 ? true : false
      );

      // Set marketer-related fields
      setSelectedMarketer(productArea.marketer);
      setSelectedMarketerId(productArea.marketer.id);

      // Set demographic includes/excludes
      setSelectedDemographicInclude({ title: productArea.demographic_include });
      setSelectedDemographicExclude({ title: productArea.demographic_exclude });
      setSelectedDemographicExclude2({
        title: productArea.demographic_exclude_2,
      });

      // Common demographic selection
      const demographicData = {
        demographicInclude: Array.isArray(productArea?.demographic_includes_id)
          ? productArea.demographic_includes_id
          : [],
        demographicExclude: Array.isArray(productArea?.demographic_excludes_id)
          ? productArea.demographic_excludes_id
          : [],
        demographicExclude2: Array.isArray(
          productArea?.demographic_exclude_2_id
        )
          ? productArea.demographic_exclude_2_id
          : [],
      };
  ...
  const handleFormSubmit = () => {
    ...
    //   return;
    // }
    if (!formDetails.marketer_id) {
      toast.error("Marketer is required");
      return;
    }
    ...
    const data = {
      product_area: formDetails,
    };
    console.log("data", data);
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_area/update`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        toast.success("Product Area Information Updated Successfully");
        setTimeout(() => {
          navigate("/product-area-list");
        }, 2000);
      })
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {!showDemographicExclude2 && (
                    <IconButton
                      onClick={toggleDemographicExclude}
              ...
              <div
                className={`${classes.dflex} ${classes.flexdirectioncolumn}  ${
                  showDemographicExclude2 ? classes.w24_5 : classes.w24_5
                }`}
              ></div>
            </div>
            {showDemographicExclude2 && (
                    ...
                    {selectedDemographicExclude2.title === "District" && (
                      ...
                      >
                        ...
                        <Autocomplete
                          ...
                          getOptionLabel={(option) => option.district_name}
                          value={district.filter((sub) =>
                            selectedDistrict.demographicExclude2.includes(
                              sub.id
                            )
                          )}
                          onChange={(event, newValue) => {
...
export default EditProductAreaForm;
