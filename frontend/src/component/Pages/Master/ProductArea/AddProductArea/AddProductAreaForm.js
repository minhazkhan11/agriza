...
function AddProductAreaForm() {
  ...
  const filteredOptions2 = demographicInclude.filter((option) => {
    if (!selectedDemographicExclude) return option.id !== 1;
    return option.id > selectedDemographicExclude.id;
  });
  ...
  const handleFormSubmit = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_area/add`,
        data,
        {
          headers: {
...
export default AddProductAreaForm;
