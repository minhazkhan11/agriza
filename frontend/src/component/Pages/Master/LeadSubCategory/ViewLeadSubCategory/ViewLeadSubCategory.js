...
function ViewLeadSubCategory() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-lead-sub-category") {
      navigate("/edit-lead-sub-category", { state: rowId });
    }
  };
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/lead_sub_category/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      fetchData();
      handleClose();
      toast.success("Lead Sub Category deleted successfully");
      const updateddatatata = leadSubCategory.filter(
        (data) => data.id !== rowId
      );
      setLeadSubCategory(updateddatatata);
    } catch (error) {
      toast.error("Lead Sub Category is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/lead_sub_category/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Lead Sub Category status changed successfully", response);
        fetchData();
        toast.success("Lead Sub Category status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Lead Sub Category status:", error);
        toast.error("Lead Sub Category status is not changed");
      });
  };
  const rows = leadSubCategory.map((d) => ({
    id: d.id ? d.id : "N/A",
    code: d.id ? `LSC${d.id}` : "N/A",
    name: d.name? d.name: "N/A",
    lead_name: d.lead_category.name? d.lead_category.name: "N/A",
  ...
  const columns = [
    ...
    {
      field: "name",
      headerName: "Lead Sub Category Name",
      headerClassName: "super-app-theme--header",
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search Lead sub category name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
...
export default ViewLeadSubCategory;
