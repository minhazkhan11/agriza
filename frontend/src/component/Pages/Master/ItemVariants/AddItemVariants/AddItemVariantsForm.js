...
function AddItemVariantsForm() {
  ...
  const handleFormSubmit = () => {
    ...
    // Validate `effective_date` for each variant
for (let i = 0; i < variantsFormDetails.length; i++) {
  const variant = variantsFormDetails[i];

  if (!variant.effective_date || isNaN(new Date(variant.effective_date))) {
    toast.error(`Please enter a valid effective date in variant ${i + 1}.`);
    return;
  }
}
