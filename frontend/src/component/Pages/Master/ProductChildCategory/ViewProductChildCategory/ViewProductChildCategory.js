...
function ViewProductChildCategory() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-product-child-category") {
      navigate("/edit-product-child-category", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      ...
      setProductSubCategory(response.data.product_child_category);
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_child_category/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      fetchData();
      handleClose();
      toast.success("Product Child Category deleted successfully");
      const updateddatatata = productSubCategory.filter(
        (data) => data.id !== rowId
      );
      setProductSubCategory(updateddatatata);
    } catch (error) {
      toast.error("Product Child Category is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_child_category/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log(
          "Product Child Category status changed successfully",
          response
        );
        fetchData();
        toast.success("Product Child Category status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Product Child Category status:", error);
        toast.error("Product Child Category status is not changed");
      });
  };
  const rows = productSubCategory.map((d) => ({
    id: d.id ? d.id : "N/A",
    code: d.id ? `PSC${d.id}` : "N/A",
    category_name: d.product_child_category_name
      ? d.product_child_category_name
      : "N/A",
    product_class_name: d.Product_sub_category_id
      ? d.Product_sub_category_id?.product_sub_category_name
      : "N/A",
  ...
  const columns = [
    ...
    {
      field: "category_name",
      headerName: "Product Child Category Name",
      headerClassName: "super-app-theme--header",
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search product child category name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
...
export default ViewProductChildCategory;
