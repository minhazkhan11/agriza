...
function ViewProductCategory() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-product-category") {
      navigate("/edit-product-category", { state: rowId });
    }
  };
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_category/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      toast.error("Product Category is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/product_category/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Product Category status changed successfully", response);
        fetchData();
        toast.success("Product Category status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Product Category status:", error);
        toast.error("Product Category status is not changed");
      });
  };
  const rows = productCategory.map((d) => ({
    id: d.id ? d.id : "N/A",
    category_name: d.category_name ? d.category_name : "N/A",
    // product_class_name: d.productClass.class_name
    //   ? d.productClass.class_name
    //   : "N/A",
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search product category name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
