...
function ViewDeliveryPoint() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-delivery-point") {
      navigate("/edit-delivery-point", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/delivery_point`,
        {
          headers: {
      ...
      setWareHouse(response.data.delivery_point_Details);
      console.log(response.data.delivery_point_Details)
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    ...
    try {
      ...
      toast.error("Delivery Point  is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/delivery_point/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Delivery Point status changed successfully", response);
        fetchData();
        toast.success("Delivery Point  status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Delivery Point  status:", error);
        toast.error("Delivery Point  status is not changed");
      });
  };

  const rows = wareHouse.map((d) => ({
    id: d.id ? d.id : "N/A",
    code: d.id ? `DP${d.id}` : "N/A",
    businessName : d.business_name  ? d.business_name : "N/A",
    GSTNumber: d.gst_no ? d.gst_no : "N/A",
    warehouseName: d.warehouse_name ? d.warehouse_name : "N/A",
    warehouseSupervisorName: d.scm_person_name ? d.scm_person_name : "N/A",
    mobileNumber: d.mobile_no ? d.mobile_no : "N/A",
  ...
  const columns = [
    ...
    {
      field: "warehouseSupervisorName",
      headerName: "Warehouse Supervisor Name",
      headerClassName: "super-app-theme--header",
      headerAlign: "center", // Align the column header to center
      width: 250,
      sortable: true,
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-delivery-point", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search Delivery Point Name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
...
export default ViewDeliveryPoint;
