...
function AddDeliveryPointForm() {
  ...
  const handleFormSubmit = () => {
    ...
    formData.append("delivery_point", JSON.stringify(data));
    // formData.append("signatureandseal", files.signature);

    axios
      .post(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/delivery_point/add`,
        formData,
        {
          headers: {
  ...
  const handleImage = async (e, field, index) => {
    ...
    try {
      const formData = new FormData();
      const entitytype = "delivery_point";
...
export default AddDeliveryPointForm;
