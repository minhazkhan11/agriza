...
function ViewVendorCategory() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-vendor-category") {
      navigate("/edit-vendor-category", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/vender_category`,
        {
          headers: {
      ...
      setVendorCategory(response.data.vender_category);
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/vender_category/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      const updateddatatata = vendorCategory.filter((data) => data.id !== rowId);
      setVendorCategory(updateddatatata);
    } catch (error) {
      toast.error("Vendor Category is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/vender_category/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Vendor Category status changed successfully", response);
        fetchData();
        toast.success("Vendor Category status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Vendor Category status:", error);
        toast.error("Vendor Category status is not changed");
      });
  };
  const rows = vendorCategory.map((d) => ({
    id: d.id ? d.id : "N/A",
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-vendor-category", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search Vendor category name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
