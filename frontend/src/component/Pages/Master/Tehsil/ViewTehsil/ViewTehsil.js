...
function ViewTehsil() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-tehsil") {
      navigate("/edit-tehsil", { state: rowId });
    }
  };
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/tehsil/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      const updateddatatata = tehsil.filter((data) => data.id !== rowId);
      setTehsil(updateddatatata);
    } catch (error) {
      toast.error("Tehsil is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/tehsil/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Tehsil status changed successfully", response);
        fetchData();
        toast.success("Tehsil status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Tehsil status:", error);
        toast.error("Tehsil status is not changed");
      });
  };

  const rows = tehsil.map((d) => ({
    id: d.id?d.id:"N/A",
    category_name: d.tehsil_name?d.tehsil_name:"N/A",
    country_code: d.district_name?d.district_name:"N/A",
           ...
           {isActive ? (
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-tehsil", cellValues.row.id);
                  }}
                >
