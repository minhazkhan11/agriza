...
function ViewLogisticArea() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-logistic-area") {
      navigate("/edit-logistic-area", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      ...
      setProductCategory(response.data.logistic_areas);
      console.log("ProductCategory", response.data.logistic_areas);
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      fetchData();
      handleClose();
      toast.success("Logistic Area deleted successfully");
      const updateddatatata = productCategory.filter(
        (data) => data.id !== rowId
      );
      setProductCategory(updateddatatata);
    } catch (error) {
      toast.error("Logistic Area is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/logistic_area/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Logistic Area status changed successfully", response);
        fetchData();
        toast.success("Logistic Area status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Logistic Area status:", error);
        toast.error("Logistic Area status is not changed");
      });
  };
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-logistic-area", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search Logistic Area Name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
...
export default ViewLogisticArea;
