...
function ViewBrandApproval() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-single-brand") {
      navigate("/edit-single-brand", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/brand/auth/pending`,
        {
          headers: {
      ...
      console.log(response.data.brand, "new product");
  ...
  const rows = brand.map((d) => ({
    id: d.id ? d.id : "N/A",
    category_name: d.brand_name ? d.brand_name : "N/A",
    marketers_id: d.marketers_id?.marketer_name
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-single-brand", cellValues.row.id);
                  }}
                >
...
export default ViewBrandApproval;
