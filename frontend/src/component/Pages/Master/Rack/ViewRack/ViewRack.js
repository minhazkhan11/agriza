...
function ViewBusinessCategory() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-rack") {
      navigate("/edit-rack", { state: rowId });
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/rake`,
        {
          headers: {
      ...
      setBusinessCategory(response.data.rake);
    } catch (error) {
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/rake/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      toast.error("Rack is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/rake/status`,
        data,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
      ...
      .then((response) => {
        console.log("Rack status changed successfully", response);
        fetchData();
        toast.success("Rack status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Rack status:", error);
        toast.error("Rack status is not changed");
      });
  };

  const rows = businessCategory.map((d) => ({
    id: d.id?d.id:"N/A",
    code: d.id ? `RACK${d.id}` : "N/A",
    name: d.name?d.name:"N/A",
    warehouse: d.warehouse_id?d.warehouse_id.name:"N/A",
           ...
           {isActive ? (
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-rack", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search Rake Name",
      inputlable: "yes",
    },
  ];
  return (
