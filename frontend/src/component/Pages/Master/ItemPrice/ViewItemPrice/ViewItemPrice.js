...
function ViewItemPrice({
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "update-item-price") {
      navigate("/update-item-price", { state: rowId });
    }
  };
  ...
  const rows = attributes.map((d) => ({
    ...
    mrp: d.ex_mrp ? d.ex_mrp : "N/A",
    selling_price: d?.ex_selling_price ? d.ex_selling_price : "N/A",
    cross_price: d?.ex_cross_price ? d.ex_cross_price : "N/A",
    for_mrp: d?.for_mrp ? d.for_mrp : "N/A",
    for_selling_price: d.for_selling_price ? d.for_selling_price : "N/A",
    for_cross_price: d.for_cross_price ? d.for_cross_price : "N/A",
    moq: d.moq ? d.moq : "N/A",
effective_date1: d.for_effective_date ? d.for_effective_date : "N/A" || d.ex_effective_date ? d.ex_effective_date : "N/A",

effective_date: d.for_effective_date || d.ex_effective_date
? (() => {
  const date = new Date(d.for_effective_date || d.ex_effective_date);
          ...
          >
            ...
            >
              {/* <Button
                ...
                // }}
                onClick={() => {
                    handleButtonClick("update-item-price", cellValues.row.id);
                  }}
              >
                Update
              </Button> */}
              {console.log("cellValues.row.mrp", cellValues.row)}
              <Button
                className={classes.bluebtn}
                onClick={() => {
                  if (cellValues.row.mrp === "N/A") {
                    handleButtonClick("update-item-price", cellValues.row.id);
                  } else {
                    handlePricePopUp();
                    setRowId(cellValues.row.id);
                    setPopupState({
                      ex: cellValues.row.ex,
                      for: cellValues.row.for,
                    });
                  }
                }}
              >
                {cellValues.row.mrp === "N/A" ? "Add" : "Update"}
              </Button>
            </Typography>
          </div>
        );
