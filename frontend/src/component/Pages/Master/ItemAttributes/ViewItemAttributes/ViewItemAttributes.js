...
function ViewItemAttributes() {
  ...
  const handleButtonClick = (type, rowId) => {
    if (type === "edit-item-attribute") {
      navigate("/edit-item-attribute", { state: rowId });
    }
  };
  ...
  const deleteDataOfRow = async (rowId) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/v1/admin/attributes/${rowId}`,
        {
          headers: {
            Authorization: `Bearer ${decryptedToken}`,
          },
      ...
      toast.error("Attributes is not deleted");
  ...
  const handleStatus = (iddd, currentStatus) => {
    ...
    axios
      .put(`${process.env.REACT_APP_API_BASE_URL}/v1/admin/attributes/status`, data, {
        headers: {
          Authorization: `Bearer ${decryptedToken}`,
        },
      })
      .then((response) => {
        console.log("Attributes status changed successfully", response);
        fetchData();
        toast.success("Attributes status changed successfully");
        handleClose();
      })
      .catch((error) => {
        console.error("Error changed Attributes status:", error);
        toast.error("Attributes status is not changed");
      });
  };

  const rows = attributes.map((d) => ({
    id: d.id ? d.id : "N/A",
    code: d.id ? `ATTR${d.id}` : "N/A",
    category_name: d.attribute_name ? d.attribute_name : "N/A",
    country_code: d.variant_details ? d.variant_details.map(item => item.variant).join(", ") : "N/A",
          ...
          >
            ...
            >
              <Paper>
                ...
                <IconButton
                  onClick={() => {
                    handleButtonClick("edit-item-attribute", cellValues.row.id);
                  }}
                >
  ...
  const Heading = [
    {
      id: 1,
      inputplaceholder: "Search Item Attribute Name",
      // export: "yes",
      inputlable: "yes",
    },
  ];
...
export default ViewItemAttributes;
